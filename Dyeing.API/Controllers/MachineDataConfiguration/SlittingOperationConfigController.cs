...
{
    public class SlittingOperationConfigController : ApiController
    {
        CommonModel.Response _res;

        [HttpPost]
        public async Task<IHttpActionResult> SaveUpdate(SlittingMcOperationConfigDataModel _obj)
        {
            _res = new CommonModel.Response();
            try
            {
                SlittingMcOperationConfigDataModel queryData = new McOperationConfigModel().McSlittingConfiguration_SaveUpdate(_obj);
                _res.response = true;
        ...
        public async Task<IHttpActionResult> GetBatchRelatedData(int batchId)
        {
            try
            {
                var queryData = new McOperationConfigModel().GetSlittingBatchRelatedData(batchId);
