app.factory("NewMcOperationConfigFactory", ['$http', function ($http) {
    var _obj = {};
    ...
    /*updated by hillol for those extra machine*/
    _obj.NewMachineConfigDataSaveUpdate = function (obj, cb) {
        $http.post(baseApiURL + 'McOperationConfigNew/McOperationConfigSaveUpdateNew', obj).then(function successCallback(response) {
            cb(response.data);
        }, function errorCallback(response) {
            alert("Error Occured during Load Information....");
        });
    ...
    _obj.GetDataByBatchIdDetailNew = function (batchId, compTime, batchType, status, cb) {
        $http.get(baseApiURL + 'McOperationConfigNew/McOperationConfigGetDataNew?BpmId=' + batchId + '&&compTime=' + compTime + '&&batchType=' + batchType + '&&status=' + status).then(function successCallback(response) {
            cb(response.data);
        }, function errorCallback(response) {
            alert("Error Occured during Load Information....");
        });
