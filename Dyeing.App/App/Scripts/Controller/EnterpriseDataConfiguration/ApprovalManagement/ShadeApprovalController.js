  ...
  function($scope, $window, $rootScope, $mdDialog, ApprovalManagement) {
    ...
    $scope.GetBodyPart = function() {
      ...
      if ($scope.batchType == "Bulk" || $scope.batchType == "Old") {
        $scope.BodyPartlist = ApprovalManagement.GetBodypart(
          $scope.Batch.BpmId,
          function(data) {
        ...
        $scope.BodyPartlist = ApprovalManagement.GetBodypartNew(
          $scope.Batch.BpmId,
          function(data) {
            $scope.BodyPartlist = data;
          }
    ...
    function defaultState() {
      debugger;
      $scope.allRollData.IHApproved = angular.isDefined(
        $scope.allRollData.IHApproved
      )
        ? $scope.allRollData.IHApproved
        : null;
      $scope.allRollData.GarmentsApproved = angular.isDefined(
        $scope.allRollData.GarmentsApproved
      )
        ? $scope.allRollData.GarmentsApproved
        : null;
      $scope.allRollData.MerchantApproved = angular.isDefined(
        $scope.allRollData.MerchantApproved
      )
        ? $scope.allRollData.MerchantApproved
        : null;
      $scope.allRollData.BuyerApproved = angular.isDefined(
        $scope.allRollData.BuyerApproved
      )
        ? $scope.allRollData.BuyerApproved
        : null;
      $scope.allRollData.ComApproved = angular.isDefined(
        $scope.allRollData.ComApproved
      )
        ? $scope.allRollData.ComApproved
        : null;
    ...
    $scope.getDataByBatchId = function() {
      ...
      if ($scope.Batch.BpmId > 0) {
        ...
        if ($scope.batchType == "Bulk" || $scope.batchType == "Old") {
          $scope.BatchData = ApprovalManagement.GetShadeApprovalBatchData(
            $scope.Batch.BpmId,
            pAppTime,
            $scope.BodyPart.Id,
            function(data) {
          ...
          $scope.BatchData = ApprovalManagement.GetShadeApprovalBatchDataNew(
            $scope.Batch.BpmId,
            pAppTime,
            $scope.BodyPart.Id,
            function(data) {
    ...
    function SaveUpdate() {
      ...
      let SendObj = {
        ...
        IHApproved:
          $scope.InHouseOk && angular.isDefined($scope.InHouseOk)
            ? true
            : $scope.InHouseNotOk && angular.isDefined($scope.InHouseNotOk)
              ? false
              : null,

        GarmentsApproved:
          $scope.GAOk && angular.isDefined($scope.GAOk)
            ? true
            : $scope.GANotOk && angular.isDefined($scope.GANotOk)
              ? false
              : null,

        MerchantApproved:
          $scope.MAOk && angular.isDefined($scope.MAOk)
            ? true
            : $scope.MANotOk && angular.isDefined($scope.MANotOk)
              ? false
              : null,

        BuyerApproved:
          $scope.BAOk && angular.isDefined($scope.BAOk)
            ? true
            : $scope.BANotOk && angular.isDefined($scope.BANotOk)
              ? false
              : null,

        ComApproved:
          $scope.CA && angular.isDefined($scope.InHouseOk)
            ? true
            : $scope.CA == false && angular.isDefined($scope.InHouseNotOk)
              ? false
              : null,
      ...
      if ($scope.batchType == "Bulk" || $scope.batchType == "Old") {
        ...
        ApprovalManagement.SaveUpdateShadeApprovalDataNew(SendObj, function(
          res
        ) {
          debugger;
          if (res.ErrorMsg == null) {
