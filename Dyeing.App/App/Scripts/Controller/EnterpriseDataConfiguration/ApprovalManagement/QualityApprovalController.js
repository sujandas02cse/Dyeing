...
app.controller("QualityApproval", ['$scope', '$window', '$rootScope', '$mdDialog', '$mdToast', 'ApprovalManagement', function ($scope, $window, $rootScope, $mdDialog, $mdToast, ApprovalManagement) {
    ...
    $scope.getDataByBatchId = function () {       
        if ($scope.Batch.BpmId > 0) {
            ...
            $scope.BatchData = ApprovalManagement.GetAllTypeApprovalBatchData($scope.Batch.BpmId, 'Quality', pAppTime, function (data) {
                ...
                let CoApprove = (angular.isDefined(data.m_Item2[0].ComApproved) && data.m_Item2[0].ComApproved != null) ? data.m_Item2[0].ComApproved : null;
                $scope.CA = (CoApprove != null && CoApprove == 1) ? true : false;

                $scope.CAReason = (angular.isDefined(data.m_Item2[0].CAReason) && data.m_Item2[0].CAReason != null) ? data.m_Item2[0].CAReason : null;
                ...
                $scope.RollData = angular.forEach($scope.RollData, function (obj) {
                    if (obj.ApprovalTime === undefined || obj.ApprovalTime == null)
                        ...
                        obj.ApprovalTime = parseInt(obj.ApprovalTime);
                ...
                $scope.DefaultCheck();
                $scope.BatchData.ApprovedWeight = $scope.RollData.filter(x => x.Ok === true).reduce((accumulator, x) => accumulator + x.RollWeight, 0).toFixed(1);
    ...
    function SaveUpdate() {       
        let SaveData = $scope.RollData.filter(x => (x.Ok != null) || (x.NotOk != null));
        let New = angular.forEach(SaveData, function (obj) {

            if (obj.Ok == true)
                obj.Status = true;
            else if (obj.NotOk == true)
                obj.Status = false;
