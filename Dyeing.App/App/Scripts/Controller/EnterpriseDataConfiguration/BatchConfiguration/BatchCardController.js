app.controller("BatchCardController", [
  "$scope",
  "$rootScope",
  "$location",
  "$mdDialog",
  ...
  function(
    ...
    BatchCard.GetBatchDataById($location.search().id, reviseNo, function(data) {
      debugger;
      $scope.batch.IsCSample =
        $scope.batch.IsCSample != 1 || $scope.batch.IsCSample === undefined
          ? false
          : true;
      ...
      if (reviseNo > 0) {
        ...
        for (i = 0; i < data.m_Item3.length; i++) {
          ...
          $scope.nozzleTrolley[i].Trolley = {
            id: data.m_Item3[i].TrolleyId,
            name: data.m_Item3[i].TrolleyNo
          };
        }
      } else {
        $scope.btnSave = "Save";
        ...
        if (res.ErrorMsg == null) {
          if (res.Data.Tag != "duplicate") {
            ...
            if ($scope.batch.IsPPSample == true)
              $window.open(
                "../DashboardManagement/BatchCardReportPP?BpmId=" +
                  res.Data.BpmId +
                  "&&Format=PDF#view=FitH"
              );
            else if ($scope.batch.IsCSample == true)
              $window.open(
                "../DashboardManagement/BatchCardReportC?BpmId=" +
                  res.Data.BpmId +
                  "&&Format=PDF#view=FitH"
              );
            else
