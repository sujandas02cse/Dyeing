app.controller("BatchCardSampleController", ['$scope', '$rootScope', '$location', '$mdDialog', '$mdToast', '$q', '$parse', 'fileReader', '$window', 'BatchCard', function ($scope, $rootScope, $location, $mdDialog, $mdToast, $q, $parse, fileReader, $window, BatchCard) {
    ...
    BatchCard.GetSampleBatchDataById($location.search().id, reviseNo, function (data) {
     ...
     //   $scope.batch.IsCSample = ($scope.batch.IsCSample != 1 || $scope.batch.IsCSample === undefined) ? false : true;
        ...
        $scope.batch[0].IsPPSample = $scope.batch[0].IsPPSample == 1 ? true : false;
        $scope.batch[0].IsCSample = $scope.batch[0].IsCSample == 1 ? true : false;


        $scope.batch[0].IsDVSample = $scope.batch[0].IsDVSample == 1 ? true : false;
        $scope.batch[0].IsTestSample = $scope.batch[0].IsTestSample == 1 ? true : false;
        $scope.batch[0].IsSMSSample = $scope.batch[0].IsSMSSample == 1 ? true : false;
        ...
        if ($scope.batch[0].BuyerId && $scope.batch[0].JobId) {
            $scope.LoadStylesByBuyerAndJob($scope.batch[0].BuyerId, $scope.batch[0].JobId).then(function () {
                ...
                $scope.$applyAsync(() => {
                    ...
                    debugger;
                    $scope.batch[0].SelectedStyle = $scope.StyleList.find(item => item.StyleId == $scope.batch[0].StyleId) || null;
    ...
    function SaveUpdate() {
        ...
        for (t1 = 0; t1 < $scope.batchSpec.length; t1++) {
            ...
            bodyWiseFabics.push({ FabricId: (t1 + 1), FabricFor: $scope.batch[t1].FabricFor, FabricType: $scope.batch[t1].FabType });
        ...
        BatchCard.SampleBatchData_SaveUpdate(obj, function (res) {
            ...
            if (res.ErrorMsg == null) {
                if (res.Data.Tag != 'duplicate') {
                    ...
                    const url = `../DashboardManagement/SampleBatchCardReport?BpmId=${res.Data.BpmId}&Format=PDF&ReportType=${reportType}#view=FitH`;
                    $window.open(url);
                }
