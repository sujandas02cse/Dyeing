  ...
  function(
    ...
    $scope.LoadProcessData = function() {
      ...
      if (
        ...
        if ($scope.RMode == "RG") {
          PlanManagement.GetBatchPrepareData(
            $scope.Unit.Id,
            fromDate,
            toDate,
            function(data) {
              for (let i = 0; i < data.length; i++) {
                ...
                data
                  .filter(
                    x =>
                      x.GroupNo == groupNo && x.UniqueGuid == data[i].UniqueGuid
                  )
                  .map(function(item) {
          ...
          PlanManagement.GetBatchReprocessData(
            $scope.Unit.Id,
            fromDate,
            toDate,
            function(data) {
    ...
    $scope.PPorC = function(type) {
      if (type === "IsPPSample") {
        ...
        $scope.StockData[0].PPSample = false;
        $scope.StockData[0].China = false;
        $scope.StockData[0].PPSamplewithChina = $scope.PPSamplewithChina
          ? false
          : true;
      }
    };
    ...
    $scope.BatchBehavior = function(f) {
      if (f == "Y" && $scope.chkYes == true) {
        ...
        if ($scope.RMode == "RG")
          $scope.BatchPrepareData = $scope.BatchPrepareDataAll.filter(
            x => x.BatchNo != undefined
          );
        else if ($scope.RMode == "RP")
          $scope.BatchReprocessData = $scope.BatchReprocessDataAll.filter(
            x => x.BatchNo != undefined
          );
    ...
    $scope.SaveRevise = function() {
      ...
      PlanManagement.BatchPrepare_SaveUpdateNew(obj, function(res) {
        ...
        if ($scope.StockData[0].PPSample === true) reportType = "PPSample";
        else if ($scope.StockData[0].China === true) reportType = "China";
    ...
    $scope.getRowClass = function($index) {
      if ($index == 0) {
        ...
        if (batchNo != prevBatchNo) {
          if (colorClass == "table-white") colorClass = "table-success";
          else colorClass = "table-white";
        }
        return colorClass;
      }
    };
    ...
    $scope.StockCheckPopup = function(ev, groupNo, IsPrepared) {
      $scope.StockData = $scope.BatchPrepareData.filter(
        x => x.GroupNo == groupNo
      );
      ...
      $scope.StockData.filter(x => x.BodyPart.includes("Body")).map(function(
        item
      ) {
        body += parseFloat(item.PlanQty == "" ? 0 : item.PlanQty);
      });
      $scope.StockData.filter(x => x.BodyPart.includes("Neck")).map(function(
        item
      ) {
        neck += parseFloat(item.PlanQty == "" ? 0 : item.PlanQty);
      });
      $scope.StockData.filter(x => x.BodyPart.includes("BNT")).map(function(
        item
      ) {
