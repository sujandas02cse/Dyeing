app.controller("FloorStatusDashboard", [
  "$scope",
  "$window",
  "$rootScope",
  "filterFilter",
  "$mdDialog",
  "Dashboard",
  function($scope, $window, $rootScope, filterFilter, $mdDialog, Dashboard) {
    $scope.TotalRecord = 0;
    $scope.totalweight = 0;
    $scope.DataRecord = [];
    ...
    $scope.LoadingData = function() {
      ...
      if (
        ...
        if (batchType == "Bulk") {
          ...
          Dashboard.GetFloorStatusSample(
            $scope.Unit.UnitId,
            fromDate,
            toDate,
            function(data) {
    ...
    $scope.LoadingStatusData = function() {
      ...
      if (!$scope.SelectStatus) $scope.SelectStatus = undefined;
    };
    ...
    $scope.ExportData = function(FloorStatusData) {
      if (FloorStatusData.length > 0) {
        ...
        alasql(
          'SELECT BatchNo,BuyerName,JobInfo,StyleInfo,Color,FabricType,Composition,FGSM,FDia,BodyQty, NeckQty, BNTQty, CksSend, Result, QualityTestApproved, FabricTestApproved,ShadeStatusBody, ShadeStatusNeck, ShadeStatusBNT, RFDApproved, BaseStatus, StartingTime, EndingTime,Status INTO XLSX("' +
            filename +
            '", { headers: true }) FROM ? ',
          [$scope.DataRecord]
        );
    ...
    function CountWeight(record) {
      ...
      angular.forEach(record, function(model) {
        if ($scope.batchType == "Bulk") {
          $scope.totalweight =
            $scope.totalweight + (model.NeckQty + model.BodyQty + model.BNTQty);
          ...
          $scope.totalweight =
            $scope.totalweight + (model.Fab1 + model.Fab2 + model.Fab3);
          $scope.totalweight = parseFloat($scope.totalweight.toFixed(2));
        }
      });
    }
    ...
    function getData(Status) {
      switch (Status) {
        case "Check Roll Sent":
          $scope.DataRecord = $scope.FloorStatusData.filter(
            x => x.CksSend == "Sent"
          );
          break;
        ...
        case "Dyeing Running":
          $scope.DataRecord = $scope.FloorStatusData.filter(
            x => x.BaseStatus.toLowerCase() === "dyeing running"
          );
          break;
        case "Dyeing End":
          $scope.DataRecord = $scope.FloorStatusData.filter(
            x => x.BaseStatus == "Dyeing End"
          );
          break;
        case "Slitting Running":
          $scope.DataRecord = $scope.FloorStatusData.filter(
            x => x.BaseStatus == "Slitting Running"
          );
          break;
        case "Slitting End":
          $scope.DataRecord = $scope.FloorStatusData.filter(
            x => x.BaseStatus == "Slitting End"
          );
          break;
        case "Stenter Running":
          $scope.DataRecord = $scope.FloorStatusData.filter(
            x => x.BaseStatus == "Stenter Running"
          );
          break;
        case "Stenter End":
          $scope.DataRecord = $scope.FloorStatusData.filter(
            x => x.BaseStatus == "Stenter End"
          );
          break;
        case "Dryer Running":
          $scope.DataRecord = $scope.FloorStatusData.filter(
            x => x.BaseStatus == "Dryer Running"
          );
          break;
        case "Dryer End":
          $scope.DataRecord = $scope.FloorStatusData.filter(
            x => x.BaseStatus == "Dryer End"
          );
          break;
        case "Compacting Running":
          $scope.DataRecord = $scope.FloorStatusData.filter(
            x => x.BaseStatus == "Compacting Running"
          );
          break;
        case "Compacting End":
          $scope.DataRecord = $scope.FloorStatusData.filter(
            x => x.BaseStatus == "Compacting End"
          );
          break;
        case "RFD":
          $scope.DataRecord = $scope.FloorStatusData.filter(
            x => x.RFDApproved == "OK" || x.RFDApproved == "P. OK"
          );
          break;
        default:
          $scope.DataRecord = $scope.FloorStatusData;
    ...
    $scope.customFilter = function(dataModel) {
      if ($scope.SelectStatus == "Check Roll Sent") {
        ...
        return (
          dataModel.RFDApproved == "OK" || dataModel.RFDApproved == "P. OK"
        );
      } else {
