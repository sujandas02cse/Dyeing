app.controller("DataConfigurationController", ['$scope', '$rootScope', '$mdDialog', '$mdToast', '$q', '$parse', 'fileReader', '$window', 'DataConfiguration', function ($scope, $rootScope, $mdDialog, $mdToast, $q, $parse, fileReader, $window, DataConfiguration) {
    ...
    DataConfiguration.GetUnitAll($rootScope.UserId, function (data) {
        $scope.UnitList = data;
    ...
    $scope.LoadBatchData = function (Unit) {
        ...
        DataConfiguration.GetBatchByUnit(Unit.Id, $scope.Mode, function (data) {
    ...
    $scope.LoadBatchDetailData = function (Batch) {
        ...
        DataConfiguration.GetBatchDetailByBatch(Batch.BpmId, function (data) {

            $scope.BatchData = data.m_Item3[0];
            $scope.Unit = $scope.UnitList.find(x => x.Id === data.m_Item3[0].UnitId);
            $scope.Batch = $scope.BatchList.find(x => x.BpmId === data.m_Item3[0].BpmId);
            ...
            .then(function (mode) {
                if (mode == "Update" || mode == "Save") {
                    ...
                    if (
                        $scope.Batch.BpmId === '' &&
                        (
                            ($scope.Type === 'Order' && $scope.Order.OrderId === undefined) ||
    ...
    function SaveUpdate() {
        ...
        DataConfiguration.SaveUpdateData(Obj, function (data) {
            $scope.Refresh();
            if (data[0].msg != '')
