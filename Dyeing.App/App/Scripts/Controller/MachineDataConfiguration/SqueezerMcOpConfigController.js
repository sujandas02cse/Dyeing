app.controller("SqueezerMcOpConfigController", ["$scope", "$rootScope", "$mdDialog", "$mdToast", "$q", "$parse", "fileReader", "$window", "NewMcOperationConfigFactory", function (
    ...
    function loadMachineList() {
        if ($scope.Unit) {
            NewMcOperationConfigFactory.GetFinMcByTypeUnitWise("Squeezer", $scope.Unit.UnitId, function (dataNew) {
    ...
    $scope.getDataByBatchId = function () {
        ...
        debugger
        NewMcOperationConfigFactory.GetDataByBatchIdDetailNew(batchId, pCompTime, status, "Squeezer", function (data) {
            debugger
