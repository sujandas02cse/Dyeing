app.controller("SuedingMcOpConfigController", ["$scope", "$rootScope", "$mdDialog", "$mdToast", "$q", "$parse", "fileReader", "$window", "NewMcOperationConfigFactory", function (
    ...
    function loadMachineList() {
        if ($scope.Unit) {
            NewMcOperationConfigFactory.GetFinMcByTypeUnitWise("Sueding", $scope.Unit.UnitId, function (dataNew) {
    ...
    $scope.getDataByBatchId = function () {
        ...
        debugger
        NewMcOperationConfigFactory.GetDataByBatchIdDetailNew(batchId, pCompTime, status, "Sueding", function (data) {
            debugger
