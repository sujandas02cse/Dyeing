app.controller("SuedingMcOpConfigController", [
  "$scope",
  "$rootScope",
  "$mdDialog",
  "$mdToast",
  ...
  function(
    ...
    function loadBatchList() {
      if ($scope.Unit && $scope.batchType) {
        NewMcOperationConfigFactory.GetBatchNoListUnitStatusWise(
          $scope.batchType,
          $scope.Unit.UnitId,
          function(dataNew) {
    ...
    function loadMachineList() {
      if ($scope.Unit) {
        NewMcOperationConfigFactory.GetFinMcByTypeUnitWise(
          "Sueding",
          $scope.Unit.UnitId,
          function(dataNew) {
    ...
    $scope.getDataByBatchId = function() {
      ...
      debugger;
      NewMcOperationConfigFactory.GetDataByBatchIdDetailNew(
        batchId,
        pCompTime,
        status,
        "Sueding",
    ...
    function SaveUpdate() {
      var obj = {
        Operation: "Sueding",
        OpTime:
          $scope.McOperation.CompTime === 0 ? 1 : $scope.McOperation.CompTime,
      ...
      NewMcOperationConfigFactory.NewMachineConfigDataSaveUpdate(obj, function(
        data
      ) {
        debugger;
