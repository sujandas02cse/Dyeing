app.controller("BrushMcOpConfigController", ["$scope", "$rootScope", "$mdDialog", "$mdToast", "$q", "$parse", "fileReader", "$window", "NewMcOperationConfigFactory", function (
    ...
    function loadMachineList() {
        if ($scope.Unit) {
            NewMcOperationConfigFactory.GetFinMcByTypeUnitWise("Brush", $scope.Unit.UnitId, function (dataNew) {
    ...
    $scope.getDataByBatchId = function () {
        ...
        debugger
        NewMcOperationConfigFactory.GetDataByBatchIdDetailNew(batchId, pCompTime, status, "Brush", function (data) {
            debugger
